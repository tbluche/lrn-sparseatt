...
from lrn_sparseatt.attention import masked_attention, sparse_attention_nested

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
    filename="profiles/2_profile.log",
    filemode="w",
)
...
def profile_attention(
    ...
    nested_indices = boolean_mask_to_nested_indices(bool_mask)
    ...
    with profile_and_report("sparse") as prof:
        for _ in range(N_RUNS):
            sparse_attention_nested(q, k, v, nested_indices)
            prof.step()
...
if __name__ == "__main__":
    ...
    print(table)
    with open("profiles/2_profile_results.md", "w") as f:
        f.write(table)
