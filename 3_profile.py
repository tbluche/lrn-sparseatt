...
logging.basicConfig(
    ...
    filename="profiles/3_profile.log",
    filemode="w",
)
...
def profile_attention(
    ...
    return dense_time / 1e6, sparse_time / 1e6, sparse_1_time / 1e6
...
def run_profiles():
    ...
    for case_num, (seq_len, d_model, n_heads, sparsity) in enumerate(
        ...
        dense_time, sparse_time, sparse_1_time = profile_attention(
            seq_len, d_model, n_heads, sparsity
        )
        rows.append(
            {
...
if __name__ == "__main__":
    ...
    print(table)
    with open("profiles/3_profile_results.md", "w") as f:
        f.write(table)
